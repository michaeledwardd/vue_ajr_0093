    ...
    <v-card>
      ...
      <v-data-table :headers="headers" :items="pegawais" :search="search">
        ...
        <template v-slot:[`item.foto_pegawai`]="{ item }">
          <v-img
            :src="$baseUrl + '/storage/' + item.foto_pegawai"
            height="100px"
            width="100px"
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <v-menu offset-y>
            ...
            <v-list>
              <v-list-item-title>
                <v-btn @click="editHandler(item)"> Edit Pegawai </v-btn>
              </v-list-item-title>

              <v-list-item-title>
                <v-btn @click="showHandler(item)"> Foto Pegawai </v-btn>
              </v-list-item-title>
            </v-list>
          </v-menu>
        </template>
          ...
          <v-container>
            <v-flex align-center>
              <v-img
                ...
                :src="
                  previewImageUrl == ''
                    ? $baseUrl + '/storage/' + form.foto_pegawai
                    : previewImageUrl
                "
                id="previewImage"
                class="mb-5"

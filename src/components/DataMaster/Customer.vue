    ...
    <v-card>
      ...
      <v-data-table :headers="headers" :items="customers" :search="search">
        ...
        <template v-slot:[`item.upload_berkas`]="{ item }">
          <v-img
            :src="$baseUrl + '/storage/' + item.upload_berkas"
            height="100px"
            width="100px"
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <v-menu offset-y>
            ...
            <v-list>
              <v-list-item-title>
                <v-btn @click="editHandler(item)"> Edit Customer </v-btn>
              </v-list-item-title>

              <v-list-item-title>
                <v-btn @click="showHandler(item)"> Berkas Customer </v-btn>
              </v-list-item-title>
            </v-list>
          </v-menu>
        </template>
          ...
          <v-container>
            <v-flex align-center>
              <v-img
                ...
                :src="
                  previewImageUrl == ''
                    ? $baseUrl + '/storage/' + form.upload_berkas
                    : previewImageUrl
                "
                id="previewImage"
                class="mb-5"
